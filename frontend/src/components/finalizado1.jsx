...
function CongratulationsScreen({
  ...
  return (
    <div
      className={`w-full h-full flex flex-col items-center font-poppins p-10 ${
    ...
    >
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center">
        <h1
          ...
          style={{
            ...animationStyle(0.2),
            backgroundImage: "linear-gradient(90deg, #b081ff, #849dff)",
        ...
        <div
          className="flex flex-col gap-2 mb-8 text-lg text-center"
        ...
        >
          ...
          >
            <CountUp end={errorCount} duration={1000} start={startCounting} />{" "}
            ERROS
          </p>
        ...
        {/* Robô flutuando */}
        <img
          src={robotCongratsImage}
          alt="Robô Festejando"
          className="w-40 h-40 sm:w-48 sm:h-48"
          style={{
            ...animationStyle(0.8),
            animationName: "float, fadeInUp",
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center">
        <div className="relative mb-4">
          ...
          <img
            ...
            style={
              isAnimating
                ? { animation: "ignite 1.5s ease-in-out forwards" }
                : {
                    filter: "grayscale(1)",
                    transform: "scale(0.8)",
                    opacity: 0.5,
          ...
          />
          {/* O contador da sequência */}
          {isAnimating && (
            <div
              className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg"
              style={{
        ...
        <p
          ...
          style={{
            animation: isAnimating
              ? "fadeInUp 0.5s ease-out 1.5s forwards"
              : "none",
            opacity: 0,
            backgroundImage: "linear-gradient(90deg, #b081ff, #849dff)",
            WebkitBackgroundClip: "text",
        ...
        >
          Continue praticando todos os dias para
          <br />
...
function AchievementRewardScreen({ onContinue, unlockedLessonId }) {
  ...
  const unlockedIndex = achievements.findIndex(
    (ach) => ach.id === unlockedLessonId
  );
  const unlockedAchievement = achievements[unlockedIndex] ?? achievements[0];
  ...
  useEffect(() => {
    ...
    const currentProgressPercent =
      ((unlockedIndex + 1) / totalAchievements) * 100;
    ...
    const shakeTimer = setTimeout(() => {
      setIsShaking(true);
    }, 3000); // Começa a tremer o cadeado
    const unlockTimer = setTimeout(() => {
      setIsShaking(false);
      setIsUnlocked(true);
    }, 4500); //  Para de tremer e atualiza a UI
    const modalShowTimer = setTimeout(() => {
      setShowModal(true);
    }, 4800); //  Mostra o pop-up do emblema
  ...
  return (
    <div
      className={`w-full h-full flex flex-col items-center font-poppins p-10 relative overflow-hidden ${
        theme === "escuro" ? "bg-gray-800" : "bg-white"
      }`}
    >
      {showModal && (
        <div
          style={{
            animation: "glow-pop-in 0.7s forwards, fade-out 0.5s 2.2s forwards",
          }}
          className="absolute inset-0 flex flex-col items-center justify-center z-20"
        >
          <p className="text-4xl font-bold mb-2" style={{ color: "#8A2BE2" }}>
            {unlockedAchievement.title}
          </p>
          <p className="text-3xl font-bold mb-4" style={{ color: "#FFFF00" }}>
              ...
              >
                {index <= unlockedIndex ? (
                  ...
                  <img
                    ...
                    style={{
                      animation:
                        index === unlockedIndex + 1 && isShaking
                          ? "shake 1.5s"
                          : "none",
                    }}
                  />
...
function LcoinsScreen({ onContinue, isFirstTime }) {
  ...
  useEffect(() => {
    ...
    if (isFirstTime) {
      ...
      console.log(
        "finalizado1.jsx: Não é a primeira vez, Lcoins já foram dadas."
      );
    }
    ...
    >
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center">
        ...
        <img
          ...
          style={{
            animation: startAnimations
              ? "coin-drop-glow 1.2s cubic-bezier(0.68, -0.55, 0.27, 1.55) forwards"
              : "none",
          }}
        />
        ...
        <p
          ...
          style={{
            animation: startAnimations
              ? "fadeInUp 0.5s ease-out 1s forwards"
              : "none",
            opacity: 0,
          }}
        >
...
export default function Finalizado1() {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {screen === "sequence" && (
        <SequenciaAtivaScreen onContinue={handleNavigation} />
      )}
      ...
      {screen === "lcoins" && (
        <LcoinsScreen onContinue={handleNavigation} isFirstTime={isFirstTime} />
      )}
    </div>
  );
}
