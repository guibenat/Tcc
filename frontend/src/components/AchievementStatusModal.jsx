...
const achievementImages = {
  "comecar-do-zero": achievementBadgeImage,
};
...
export function AchievementStatusModal({ onClose }) {
  ...
  console.log(
    `ACHIEVEMENT_MODAL: Lendo progresso 'comecar-do-zero': ${completedSteps} / ${totalSteps}`
  );
  ...
  if (completedSteps >= totalSteps && totalSteps > 0) {
    ...
    console.log(
      `ACHIEVEMENT_MODAL: 'comecar-do-zero' NÃO desbloqueado. (Progresso ${completedSteps} < ${totalSteps})`
    );
  }
  ...
  // Estado para animar a barra de progresso
    ...
    >
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center w-full max-w-5xl">
        ...
        <div style={{ animation: "fadeInUp 0.5s forwards 1s", opacity: 0 }}>
          ...
          >
            {unlockedCount > 0
              ? "Belo começo! Continue assim para desbloquear mais!"
              : "Sua jornada está só começando. Vamos lá!"}
          </p>
          <img
            src={robotIconImage}
            alt="Ícone Robô"
        ...
        <div className="w-full relative mt-12">
          ...
          {/* Renderiza os ícones de conquista*/}
          <div className="absolute -bottom-8 w-full flex justify-between px-2">
            {allAchievements.map((ach) => {
              ...
              const imageSrc =
                isAchUnlocked && achievementImages[ach.id]
