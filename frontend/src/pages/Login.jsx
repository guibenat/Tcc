...
import { handleEmailLogin, handleGoogleAuth } from '../authLogic'; // Nosso novo arquivo

// --- Ícones (GoogleIcon, EyeIcon, FooterText) ---
const GoogleIcon = () => (
...
export default function Login() {
    ...
    // 2. LÓGICA DE LOGIN COM E-MAIL
    const handleLogin = () => {
        setError('');
        
        // Validações locais (e-mail vazio, formato)
        if (!email || !password) {
        ...
        // Chama a lógica de auth centralizada
        const loginError = handleEmailLogin(email, password);
        
        if (loginError) {
            setError(loginError); // Mostra o erro (ex: "Senha incorreta")
        } else {
            // Sucesso. Inicia a animação de saída.
    ...
    const googleLogin = useGoogleLogin({
        // O 'onSuccess' é chamado quando o pop-up do Google fecha com sucesso
        onSuccess: async (tokenResponse) => {
            try {
                // Pega o token de acesso que o Google nos deu
                const accessToken = tokenResponse.access_token;
                
                // Usa o token para buscar os dados do perfil do usuário no Google
                ...
                // Chama nossa lógica de auth centralizada com os dados do Google
                ...
                <main className="w-screen z-10 flex-grow flex flex-col justify-center items-center px-4 py-20">
                    <div className={`content-box w-full max-w-md p-8 sm:p-12 rounded-2xl shadow-lg ${exitAnimationClass} ${enterAnimationClass} ${
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <button 
                                onClick={() => googleLogin()}
                                className={`flex items-center justify-center w-full py-3 rounded-xl border transition-colors ${
                                theme === 'escuro' 
