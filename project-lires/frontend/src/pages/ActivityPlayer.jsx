...
// --- IMPORTAÇÃO DAS LIÇÕES E ASSETS ---
...
// --- Componentes Visuais (BottomNotification, HeartIcon, etc...) ---
// (Nenhuma alteração aqui, eles permanecem iguais)
...
export default function ActivityPlayer() {
    ...
    // 1. PEGAMOS 'setCompletedLessons' DO CONTEXTO
    const { lives, setLives, setCompletedLessons } = useSettings(); 
    ...
    // (O estado de 'lives' foi removido, agora é global)
    ...
    const proceedToNextStep = () => {
        ...
        if (isRedoMode) {
            ...
            if (nextRedoIndex < incorrectSteps.length) {
                ...
                // --- 2. ADICIONADO: Marcar como concluída ---
        ...
        if (isFinalQuestion) {
            if (incorrectSteps.length > 0) {
                ...
                // --- 3. ADICIONADO: Marcar como concluída ---
                setCompletedLessons(prev => ({...prev, [lessonId]: true}));
                // Se acertou tudo, finaliza e navega
                navigate('/finalizado', { 
                    state: { 
                        errorCount: 0, 
                        totalExercises: totalSteps
    ...
    const handleCheckAnswer = () => {
        ...
        if (selectedAnswer === correctAnswer) {
            ...
            // (A lógica de 'setLives' já usa o setLives global)
            setLives(lives - 1); 
    ...
    // Lógica de Progresso (sem alteração)
    let progress;
    if (isRedoMode) {
